...
//   document.getElementById("today-date").textContent = formattedDate;
  ...
  participantsData.forEach((p) => {
    ...
    const isOvulating =
      Math.abs((today - ovulationDate) / (1000 * 60 * 60 * 24)) < 1;
    ...
    if (isMenstruating) {
      ...
      li.innerHTML = `
          ...
          ${p.name}
          <div class="tooltip">${cycleDay} день цикла (${
        p.duration - cycleDay + 1
      } день менструации)</div>
        `;
  ...
  document.getElementById("menstruation-empty").style.display = hasMenstruation
    ? "none"
    : "block";
  document.getElementById("ovulation-empty").style.display = hasOvulation
    ? "none"
    : "block";
...
function updateMoonPhase(date) {
  ...
  document.getElementById(
    "moon-phase-text"
  ).textContent = `${phase.emoji} ${phase.name}`;
}
...
const centerDateInput = document.getElementById("date-picker");
...
function drawCycleChart(centerDateStr) {
  ...
  const datasets = participantsData
    .map((girl) => {
      if (!document.getElementById(girl.name)) return null;
      if (!document.getElementById(girl.name).checked) return null;
...
function setupUnifiedCheckboxes() {
  const container = document.getElementById("unified-checkboxes");
  container.innerHTML = "";

  participantsData.forEach((girl) => {
    const label = document.createElement("label");
    label.innerHTML = `
        <input type="checkbox" id="${girl.name}" checked>
        <span class="checkbox-custom"></span>
        <span class="participant-name">${girl.name}</span>
    `;
...
function generateTimeSeries(participant, days = 365) {
  ...
  while (
    ...
    currentDate = new Date(
      currentDate.getTime() + participant.cycle * 24 * 60 * 60 * 1000
    );
  }
...
function createConclusion(analyses) {
  ...
  const selectedAverageCycle =
    analyses.reduce((sum, a) => sum + a.cycleLength, 0) / analyses.length;

  let conclusionHTML = `
      <div class="general-conclusion">
        <h3>Общий анализ (N=${analyses.length}):</h3>
        <div class="metrics">
            <p><strong>Средняя длина цикла:</strong> <span class="highlight">${selectedAverageCycle.toFixed(
              2
            )} дней</span> 
            (диапазон ${Math.min(
              ...analyses.map((a) => a.cycleLength)
            )}-${Math.max(...analyses.map((a) => a.cycleLength))} дней)</p>
            
            <p><strong>Синхронизация с лунным циклом (29.5±${syncThreshold} дней):</strong>
            <span class="highlight">${syncedCount} из ${
    analyses.length
  } (${syncPercentage}%)</span></p>
        </div>

        <div class="statistical-analysis">
            <h4>Статистический анализ:</h4>
            <p>При равномерном распределении циклов в диапазоне ${Math.min(
              ...analyses.map((a) => a.cycleLength)
            )}-${Math.max(...analyses.map((a) => a.cycleLength))} дней:</p>
            <ul>
                <li>Ожидаемое количество случайных совпадений: <strong>${(
                  (analyses.length * 4) /
                  9
                ).toFixed(1)}</strong></li>
            ...
            ${
              syncedCount >
              (analyses.length * (syncThreshold * 2)) /
                (Math.max(...analyses.map((a) => a.cycleLength)) -
                  Math.min(...analyses.map((a) => a.cycleLength)))
        ...
        <div class="interpretation">
            ...
            <div class="comparison">
                ...
                <p>Согласно мета-анализу ${
                  syncedCount > 0
                    ? "наши результаты (44.4%) близки к данным"
                    : "наши результаты расходятся с данными"
                } 
        ...
        <div class="participants-list">
            ...
            <p>${analyses
              ...
              .map(
                (a) => `
                <span class="synced-participant">${a.name} (${
                  a.cycleLength
                } дней, r=${a.moonCorrelation.toFixed(3)})</span>
            `
              )
              .join(", ")}</p>
        </div>`
  ...
  analyses.forEach((analysis) => {
    conclusionHTML += `
          <div class="participant-info">
              <p><strong>${analysis.name}</strong>: цикл ${
      analysis.cycleLength
    } дней</p>
              <p>Корреляция с лунным циклом: ${analysis.moonCorrelation.toFixed(
                3
              )}</p>
              <p>Максимальная корреляция (${analysis.maxCorrelation.toFixed(
                3
              )}) при лаге ${analysis.maxCorrelationLag} дней</p>
              <p>${
                analysis.isSynced
                  ? '<span class="highlight">Синхронизация с лунным циклом</span>'
                  : "<span>Нет синхронизации с лунным циклом</span>"
              }</p>
          </div>
      `;
  });
...
function updateCorrelationChart() {
    const selectedParticipants = participantsData
      .filter((p) => document.getElementById(p.name)?.checked)
      .map((p) => ({
        name: p.name,
...
participantsCheckbox.parentNode.insertBefore(
  buttonGroup,
  participantsCheckbox.nextSibling
);
...
/* function setupParticipantsCheckboxes() {
  ...
  `;
    ...
    label
      .querySelector("input")
      .addEventListener("change", updateCorrelationChart);
  });
} */

document
  .querySelectorAll("#participants-checkbox input")
  .forEach((checkbox) => {
    checkbox.addEventListener("change", updateCorrelationChart);
  });

selectAllBtn.addEventListener("click", () => {
  document.querySelectorAll("#unified-checkboxes input").forEach((checkbox) => {
...
const showCorrelationChartBtn = document.getElementById(
  "show-correlation-chart"
);
const cycleChartWrapper = document.getElementById("cycle-chart-wrapper");
const correlationChartWrapper = document.getElementById(
